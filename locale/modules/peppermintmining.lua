...
--Persistent data is of form { ores={}, index }
global.peppermint = {ores = {}, index = 1}
...
function peppermint.mark(event)
    ...
    if event.alt then --Remove, not add.
        ...
        for _, ore in pairs(ores) do
            for n = #global.peppermint.ores, 1, -1 do
                if ore == global.peppermint.ores[n] then
                    table.remove(global.peppermint.ores, n)
                    removed = true
                end
            end
        end
    ...
    for i = #ores, 1, -1 do
        for _, network in pairs(game.player.surface.find_logistic_networks_by_construction_area(ore.position, player.force)) do
    ...
    for i = #ores, 1, -1 do
        local dupe = false
        for n = #global.peppermint.ores, 1, -1 do
            if ores[i] == global.peppermint.ores[n] then
                table.remove(ores, i)
                dupe = true
                break
            end
        if not dupe then
            table.insert(global.peppermint.ores, ores[i])
            added = true
        end
    end
    if added then
...
--Magic happens here.
--NOTE: this does not respect forces.  One force could flag a mine for another force's roboport.
function peppermint.mine(event)
    if (game.tick + 13) % 60 ~= 0 then
        return
    end
    local size = #global.peppermint.ores
    if size == 0 then return end
    if global.peppermint.index > size then
        peppermint.cook()
        global.peppermint.index = 1
    end
    local ore = global.peppermint.ores[math.random(size)]
    if not (ore and ore.valid) then
        table.remove(global.pepper.ores, global.peppermint.index)
        --Do not advance index.
        return
    end
    local networks = ore.find_logistic_networks_by_construction_area
    if not networks then --Power died?  Roboport died?
        global.peppermint.index = global.peppermint.index + 1
        return
    end
    local network
    ...
    surface.pollute(position, global.peppermint.pollution * count * cargo_multiplier)
...
function peppermint.cook()
    local n = #global.peppermint.ores
    while n > 2 do
        local k = math.random(n)
        global.peppermint.ores[n], global.peppermint.ores[k] = global.peppermint.ores[k], global.peppermint.ores[n]
        n = n - 1
    end
end
