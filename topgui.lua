...
function topgui_remove_button(player_name, button_name)
	...
	bf = mod_gui.get_button_flow(game.players[player_name])
	bf.topgui[button_name].destroy()
end
...
function topgui_gui_changed(p)
	...
	for i, button in pairs(global.topgui.sorted[p.name]) do
		b = bf.topgui.add {name=button.name, type="button", caption=button.caption}
		if button.color ~= nil then
			b.style.font_color = button.color
		end
	end
	bf.topgui.style = "slot_table_spacing_flow_style"
end
...
Event.register(defines.events.on_player_joined_game, function(event)
	...
	global.topgui[p.name] = global.topgui[p.name] or {}
...
-- topgui_add_button(p.name, new_button1)
